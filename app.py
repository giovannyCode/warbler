...
def do_logout():
    ...
    if CURR_USER_KEY in session:
        ...
        flash("You have suscesfully logout Goodbye!", "info")
        return redirect('/')
...
def show_liked(user_id):
    """Show list of messages user has liked"""
...
def profile():
    ...
    if CURR_USER_KEY not in session and user.username != session.get('CURR_USER_KEY'):
    ...
    if form.validate_on_submit():
        ...
        userAuthenticate = User.authenticate(form.username.data,form.password.data)
        if userAuthenticate:
            ...
            return redirect(f'/users/{user.id}')
             ...
             flash(f"Password does not match, {user.username}!", "danger")
             return redirect('/')
    else:
...
def homepage():
    ...
    if g.user:
        following_ids = [folower.id for folower in g.user.following]
